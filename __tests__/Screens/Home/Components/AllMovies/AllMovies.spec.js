...
import {
  ...
  handleEndReached,
  handleKeyExtractor,
} from 'MoviesApp/src/Screens/Home/Components/AllMovies/AllMovies';
import {shallow} from 'enzyme';
import {AllMovies} from 'MoviesApp/src/Screens/Home/Components';
...
describe('rendering movie', () => {
  it('should render movie card correctly', () => {
    ...
    expect(shallow(renderMovie(props))).toMatchSnapshot();
  });
});
...
describe('Movies List Key Extractor', () => {
  it('should return unique key for movie  component', () => {
    ...
    const expectedMoviesUniqueKeys = ['3450', '345121', '299292'];
    const result = movies.map((movie, index) =>
      handleKeyExtractor(movie, index),
    );
